...
const CenterPage: React.FC<Props> = ({ selectedThread }) => {
  ...
  const [isCustomMailOpen, setIsCustomMailOpen] = useState(false);

  const togglePopUp = () => {
    setShowPopUp(prev => !prev);
    setIsCustomMailOpen(prev => !prev);
  };
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      if (isCustomMailOpen) {
        event.preventDefault(); // Prevent default behavior when CustomMail is open
        return; // Ignore key events if CustomMail is open
      }
  ...
  return (
    <div className="overflow-y-scroll no-scrollbar h-full">
      ...
      <div className="mx-8">
        {showPopUp && (
          <CustomMail
            threadId={Number(selectedThread)}
            onClose={() => {
              setShowPopUp(false);
              setIsCustomMailOpen(false); // Reset the state when closing CustomMail
            }}
          />
        )}
      </div>
